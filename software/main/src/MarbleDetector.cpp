...
void MarbleDetector::_convert_eighth_note_values_to_marble_types(uint8_t eighth_note_index)
{
    ...
    {
        marble_type_t marble_type = _get_marble_type(i, _get_thresholds_for_eighth_note(eighth_note_index, i));
...
const uint16_t *MarbleDetector::_get_thresholds_for_eighth_note(uint8_t eighth_note_index, uint8_t value_index)
{
...
marble_type_t MarbleDetector::_get_marble_type(uint8_t value_index, const uint16_t *threshold)
{      
    ...
    {
        if (value < threshold[marble_type])
        {
            break;
        }
    }
