...
{
    for (size_t i = 0; i < sensor_list_size; i++)
    {
        ir_sens_boards->select_sensor(sensor_list[i]);
        for (size_t j = 0; j < multisampling; j++)
        {
            if (j == 0)
            {
                values[i] = 0;
...
void IRSensReader::read_sub_board_values_on(uint32_t *values_on, const uint8_t *sensor_list, size_t sensor_list_size, int multisampling)
{
    _ir_sens_boards->enable_leds();
    #if (ONESHOT_IR_LED_RISE_MS > 0)
        vTaskDelay(pdMS_TO_TICKS(ONESHOT_IR_LED_RISE_MS));
    ...
    _read_sub_board_values(_ir_sens_boards, _adc_handle, values_on, sensor_list, sensor_list_size, multisampling);

    _ir_sens_boards->disable_leds();

    _average_multisampled_values(sensor_list_size, values_on, multisampling);
}

void IRSensReader::read_sub_board_values_off(uint32_t *values_off, const uint8_t *sensor_list, size_t sensor_list_size, int multisampling)
{
    _clean_adc_input(_ir_sens_boards, _adc_handle);
    _read_sub_board_values(_ir_sens_boards, _adc_handle, values_off, sensor_list, sensor_list_size, multisampling);
    _average_multisampled_values(sensor_list_size, values_off, multisampling);
}


void IRSensReader::read_values_on(uint32_t *values_on, int multisampling)
{
    _ir_sens_boards->enable_leds();
    #if (ONESHOT_IR_LED_RISE_MS > 0)
        vTaskDelay(pdMS_TO_TICKS(ONESHOT_IR_LED_RISE_MS));
...
void IRSensReader::read_values_off(uint32_t *values_off, int multisampling)
{
    _clean_adc_input(_ir_sens_boards, _adc_handle);
...
inline void _average_multisampled_values(uint8_t num_values, uint32_t *values, int multisampling)
{
